...
public class LocationApiClient
{
    ...
    public LocationApiClient(HttpClient http)
    {
        _http = http;
    }
    ...
    {
        return await _http.GetFromJsonAsync<List<LocationVm>>("api/location") ?? new List<LocationVm>();
    }

    public async Task<LocationVm?> GetAsync(int id)
    {
        return await _http.GetFromJsonAsync<LocationVm>($"api/Location/{id}");
    }
